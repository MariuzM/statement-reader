...
export const Sidebar = ({
  ...
  return (
    <aside className="border-border sticky top-0 flex h-screen w-[266px] shrink-0 flex-col gap-6 overflow-y-auto border-r bg-[#0c0e12] px-[18px] py-[22px]">
      <div className="flex items-center gap-[11px]">
        <div className="from-accent-soft to-accent-dark flex h-[34px] w-[34px] items-center justify-center rounded-[9px] bg-linear-145 text-base font-bold text-[#06120d]">
          S
        </div>
        <div>
          <div className="text-sm font-semibold tracking-tight">Statement Reader</div>
          <div className="text-muted mt-px text-[11px]">Earnings report</div>
        </div>
      </div>

      <div className="border-border bg-panel-2 rounded-[11px] border px-[13px] py-3">
        <div className="flex items-center gap-2 overflow-hidden text-[12.5px] font-medium">
          <span className="bg-accent h-[7px] w-[7px] shrink-0 rounded-sm" />
      ...
      <div>
        <div className="text-subtle mb-[9px] px-1 text-[11px] font-semibold tracking-[0.06em] uppercase">
          Views
        </div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-[11px] rounded-[9px] px-[11px] py-[9px] text-[13.5px] font-medium transition-colors ${
                  active
                    ? 'text-text bg-white/[0.055]'
                    : 'text-muted hover:bg-white/[0.03] hover:text-[#c6cbd4]'
                }`}
              >
          ...
          {currencies.length > 1 ? (
            <div className="relative">
              <select
                ...
                className="border-border bg-panel-2 text-text focus:border-accent w-full cursor-pointer appearance-none rounded-[9px] border px-3 py-[9px] text-[13px] font-medium outline-none"
              >
                {currencies.map((c) => (
              ...
              <span className="text-subtle pointer-events-none absolute top-1/2 right-3 -translate-y-1/2 text-[11px]">
                ▾
              </span>
            </div>
          ) : (
            <div className="border-border bg-panel-2 flex items-center gap-2 rounded-[9px] border px-3 py-[9px] text-[13px] font-medium">
              <span className="text-muted font-mono">{currencySymbol(currency)}</span>
              {currency}
            </div>
          )}
          <div className="text-subtle mt-[7px] px-1 text-[10.5px] leading-[1.4]">
                ...
                <button
                  ...
                  className={`rounded-lg border px-[13px] py-1.5 text-[12.5px] font-medium transition-colors ${
                    active
                      ...
                      : 'border-border bg-panel-2 hover:border-border-strong text-[#aeb4c0]'
                  }`}
                >
                  {c.label}
                </button>
      ...
      <div className="mt-auto flex flex-col gap-2.5 px-1">
        <button
          onClick={onReset}
          className="border-border bg-panel-2 hover:border-accent hover:text-text rounded-[9px] border px-3 py-2 text-[12.5px] font-medium text-[#aeb4c0] transition-colors"
        >
          Load new file
        </button>
