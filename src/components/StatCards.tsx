...
export const StatCards = ({ data, currency }: { data: Dashboard; currency: string }) => {
  const cards: {
  ...
  }[] = [
    {
      ...
      sub: data.delta ? 'vs previous year' : `${data.years.length} ${data.years.length === 1 ? 'year' : 'years'}`,
    },
    {
    ...
    },
    {
      ...
      sub: data.bestMonth ? `${MONTHS[data.bestMonth.month]} ${data.bestMonth.year}` : 'No earnings',
    },
  ]
  ...
  return (
    <div className="grid grid-cols-2 gap-3.5 md:grid-cols-4">
      {cards.map((c) => (
        <div
          key={c.label}
          className="rounded-[14px] border border-border bg-panel px-[18px] py-[17px] transition-colors hover:border-border-strong"
        >
          <div className="text-[11.5px] font-medium uppercase tracking-[0.04em] text-muted">{c.label}</div>
          <div
